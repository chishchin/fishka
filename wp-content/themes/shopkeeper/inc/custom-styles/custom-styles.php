...
function getbowtied_customizer_styles(){
	...
	/* Fonts */
	$shopkeeper_theme_options['main_font'] 									= getbowtied_theme_option('main_font', 'NeueEinstellung');		      
	...
	$shopkeeper_theme_options['secondary_font'] 							= getbowtied_theme_option('secondary_font', 'Radnika');
...
function shopkeeper_custom_styles() {	
	global 	$post, 
		...
		function getbowtied_google_fonts($default_fonts) {

			if (!in_array(getbowtied_theme_option( 'main_font', 'NeueEinstellung'), $default_fonts) || !in_array(getbowtied_theme_option( 'secondary_font', 'Radnika'), $default_fonts)): 

				$main_font 					= getbowtied_theme_option( 'main_font', 'NeueEinstellung');
				$main_font_variants 		= getbowtied_theme_option( 'main_font_variants', array('400'));

				$secondary_font 			= getbowtied_theme_option( 'secondary_font', 'Radnika');		
				...
				if (!in_array($main_font, $default_fonts) )
				{
				...
				if (!in_array($secondary_font, $default_fonts) )
				{
		...
		getbowtied_google_fonts($default_fonts);
		endif;
	...
	<style>
		...
		<?php //if ( (isset($shopkeeper_theme_options['main_font'])) && (trim($shopkeeper_theme_options['main_font']) != "" ) ) : ?>			
			...
			{ 
				font-family: 
				<?php if (!in_array($shopkeeper_theme_options['main_font'], $default_fonts)): ?>
					<?php echo '\'' . $shopkeeper_theme_options['main_font'] . '\','; ?> 
					sans-serif;
				<?php else: ?>
					<?php echo $shopkeeper_theme_options['main_font']; ?> 
				<?php endif; ?>
		...
		<?php //if ( (isset($shopkeeper_theme_options['secondary_font'])) && (trim($shopkeeper_theme_options['secondary_font']) != "" ) ) : ?>
			...
			{
				font-family: 
				<?php if (!in_array($shopkeeper_theme_options['secondary_font'], $default_fonts)): ?>
					<?php echo '\'' . $shopkeeper_theme_options['secondary_font'] . '\','; ?> 
					sans-serif;
				<?php else: ?>
					<?php echo $shopkeeper_theme_options['secondary_font'] .', sans-serif'; ?> 
				<?php endif; ?>
			}			
		<?php //endif; ?>
